...
Last updated: 2026-05-18 (Auto-trade reasoning, Risk Dial, short-selling guard, UX fixes).
...
| 8 | Glossary tab — 25 terms (added Long Position + Short Position this session) | ✅ Done |
...
## Phase 9 — Auto-Trade Intelligence (2026-05-18)

### Auto-Trade Reasoning

Every auto-trade now stores a `reasoning TEXT` column in `auto_trade_log`. Built by `_build_reasoning()` in `auto_trader.py` — deterministic, no API call. Maps each signal label / alert condition / filer source to a plain-English explanation (why + potential benefit), appends evaluator verdict and risk level context.
...
Previously: submit to Alpaca → INSERT log → commit. A crash between submit and commit left orders in Alpaca with no local record, causing them to show as "manual" in Recent Orders.
...
`POST /api/auto-trades/backfill` — retroactively marks an existing Alpaca order as auto. Looks up the order from Alpaca, inserts a row into `auto_trade_log`. Used via the `→ auto` button on manual rows in Recent Orders. Guards against double-insert (409).

### Short-Selling Guard

`allow_short_selling` setting (boolean, default `false`). When off, `maybe_auto_trade()` calls `GET /v2/positions/{symbol}` before any sell — if no long position exists (404 or qty ≤ 0), the trade is skipped. Toggle in Settings drawer.
...
reasoning      TEXT                 -- plain-English explanation of why the trade was taken
...
- `src/types/index.ts` — All types. `AutoTradeEntry` now has `recommendation`, `reasoning`, `status: 'pending' | 'submitted' | 'failed'`. `AppSettings` has `risk_level` and `allow_short_selling`.
...
| `tabs/GlossaryTab.tsx` | 25 terms in 4 sections. Long/Short Position added to Trading section. |
...
| `PositionsTable.tsx` | Grouped by sector. QTY column shows `N [LONG\|SHORT]` badge. Close button correctly uses BUY for shorts. Hover → lazy company name below ticker. |
| `AiNarratives.tsx` | Two manual-trigger cards: Portfolio Briefing + Risk Analysis. |
| `OrdersTable.tsx` | Recent orders. AUTO rows show ▼ expand for reasoning. `→ auto` backfill button on manual rows. Hover → lazy company name. Accepts `autoTrades` prop for `order_id → reasoning` lookup. |
...
| `AutoTradeLog.tsx` | Auto-trade history table with expandable reasoning. |
...
*(unchanged from previous handoff — see Phase 5 notes)*
...
1. **Auto-trade reasoning** — `reasoning TEXT` column in `auto_trade_log`. `_build_reasoning()` in `auto_trader.py` builds plain-English notes per signal/alert/filer source. Shown as expandable rows in both Auto-Trade Log and Recent Orders.
2. **Risk Dial (1–10)** — `risk_level` setting. Controls evaluator gate strength (hard/soft/bypass) and trade size multiplier (50%–200%). Slider UI in Settings with color-coded labels.
3. **Two-phase commit** — `auto_trade_log` row written as `pending` before Alpaca call, updated after. Eliminates "mystery manual" orders caused by process restarts mid-trade.
4. **Backfill endpoint** — `POST /api/auto-trades/backfill`. `→ auto` button on manual rows in Recent Orders retroactively creates a log entry so the order tags correctly.
5. **Short-selling guard** — `allow_short_selling` setting (default off). Sell signals skipped if no long position exists. Toggle in Settings.
6. **Long/Short clarity in PositionsTable** — QTY shows `N [LONG|SHORT]` badge. Close button direction fixed for shorts. Market value shown as absolute.
7. **Company name hover** — `PositionsTable` and `OrdersTable` both lazy-fetch company name on symbol hover (same pattern as MarketInsights).
8. **Glossary additions** — Long Position and Short Position added to Trading section (now 25 terms total).

### Bug Fixes

- Auto-Trade Log column headers were center-aligned (browser `<th>` default) while cells were left-aligned. Fixed with `textAlign: 'left'` on `hdr` style.
- Recent Orders column distribution — removed `tableLayout: fixed` + colgroup that gave SOURCE column ~500px. Back to natural auto-sizing.

### Files Modified

**Backend:**
- `app/auto_trader.py` — reasoning, risk dial, two-phase commit, short-selling guard
- `app/database.py` — `reasoning` column, `risk_level` + `allow_short_selling` defaults, migration
- `app/routers/settings.py` — `risk_level`, `allow_short_selling` in patch model/reader/saver; `backfill` endpoint; `reasoning`/`recommendation` in auto-trades query

**Frontend:**
- `src/types/index.ts` — `reasoning`, `recommendation`, `pending` status on `AutoTradeEntry`; `risk_level`, `allow_short_selling` on `AppSettings`
- `src/components/SettingsPanel.tsx` — Risk Dial slider, Allow Short Selling toggle
- `src/components/AutoTradeLog.tsx` — expandable reasoning rows, `pending` status, header alignment fix
- `src/components/OrdersTable.tsx` — expandable reasoning rows (from `autoTrades` prop), `→ auto` backfill button, company name hover, column layout fix
- `src/components/PositionsTable.tsx` — LONG/SHORT badge, close direction fix, company name hover
- `src/components/tabs/PositionsTab.tsx` — threads `autoTrades` + `onMarkAuto` props
- `src/components/tabs/GlossaryTab.tsx` — Long Position + Short Position terms
- `src/App.tsx` — `handleMarkAuto`, passes `autoTrades` to PositionsTab
