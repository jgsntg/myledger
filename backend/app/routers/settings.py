...
async def _read_settings(db: asyncpg.Connection) -> dict:
    rows = await db.fetch("SELECT key, value FROM system_settings")
...
@router.get("/settings")
async def get_settings(db: asyncpg.Connection = Depends(get_db)) -> dict:
...
async def update_settings(
    ...
    if body.default_trade_usd is not None:
        ...
        await db.execute(
            "INSERT INTO system_settings (key, value) VALUES ('default_trade_usd', $1) "
            "ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value",
    ...
    if body.tax_long_term_days is not None:
        ...
        await db.execute(
            "INSERT INTO system_settings (key, value) VALUES ('tax_long_term_days', $1) "
            "ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value",
    ...
    if body.insights_extra_symbols is not None:
        await db.execute(
            "INSERT INTO system_settings (key, value) VALUES ('insights_extra_symbols', $1) "
            "ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value",
    ...
    if body.risk_level is not None:
        ...
        await db.execute(
            "INSERT INTO system_settings (key, value) VALUES ('risk_level', $1) "
            "ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value",
            str(level),
        )
    ...
    if body.allow_short_selling is not None:
        await db.execute(
            "INSERT INTO system_settings (key, value) VALUES ('allow_short_selling', $1) "
            "ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value",
...
async def backfill_auto_trade(
    ...
    existing = await db.fetchrow(
        "SELECT id FROM auto_trade_log WHERE order_id = $1", body.order_id
    )
    if existing:
...
async def list_auto_trades(
    ...
    rows = await db.fetch(
        ...
        "FROM auto_trade_log ORDER BY created_at DESC LIMIT $1",
        limit,
    )
